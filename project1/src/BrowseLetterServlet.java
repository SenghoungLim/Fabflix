...
public class BrowseLetterServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            int paramIndex = 1; // Reset paramIndex to 1
            ...
            if (letter.equals("*")) {
                // Retrieve movies starting with non-alphanumeric characters (0-9, A-Z)
                queryBuilder1.append(" AND (m.title REGEXP '^[^A-Za-z0-9]') ");
            } else {
                // Retrieve movies starting with a specific letter (case-insensitive)
                queryBuilder1.append(" AND LOWER(m.title) LIKE ? ");
            }

            queryBuilder1.append("GROUP BY m.id, m.title, m.year, m.director ");
            queryBuilder1.append(") AS subquery");

            // Create a PreparedStatement for the second query
            int paramIndex1 = 1; // Reset paramIndex to 1

            PreparedStatement statement1 = dbCon.prepareStatement(queryBuilder1.toString());

            if (!letter.equals("*")) {
                statement1.setString(paramIndex1++, letter + "%");
            }
